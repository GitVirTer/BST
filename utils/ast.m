...
function [st,t,f] = ast(timeseries,minfreq,maxfreq,samplingrate,freqsamplingrate,p)
...
spe_nelements =ceil((maxfreq - minfreq+1)/freqsamplingrate);%Æµ¶Î¸öÊý
...
st = strans(timeseries,minfreq,maxfreq,samplingrate,freqsamplingrate,verbose,removeedge,analytic_signal,factor,p);
...
function st = strans(timeseries,minfreq,maxfreq,samplingrate,freqsamplingrate,verbose,removeedge,analytic_signal,factor,p);
...
if minfreq == 0
    ...
    st(1,:)=ifft(vector_fft(round(minfreq+1:minfreq+n)).*g_window(n,minfreq,factor,p));
end
...
for banana=freqsamplingrate:freqsamplingrate:(maxfreq-minfreq)
    st(round(banana/freqsamplingrate)+1,:)=ifft(vector_fft(round(minfreq+banana+1:minfreq+banana+n)).*g_window(n,minfreq+banana,factor,p));
...
%------------------------------------------------------------------------
function gauss=g_window(length,freq,factor,p)
